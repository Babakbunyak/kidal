        ...
        <div class="card shadow">
            <div class="card-header">
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    ...
                    <a href="{{ route('admin.post.index') }}" class="btn btn-success btn-sm shadow-sm">{{ __('Go Back') }}</a>
                </div>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.posts.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select name="excerpt" id="excerpt">{{ old('excerpt') }}</select>
                    </div>
                    <div class="form-group">
                        <label for="description">{{ __('Decription') }}</label>
                        <select name="description" id="description">{{ old('description') }}</select> 
                    </div>
                    <div class="form-group">
